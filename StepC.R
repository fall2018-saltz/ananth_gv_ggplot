...
bar_chart_murder_state_sorted <- ggplot() + geom_col(data = merged_data, aes(x = reorder(factor(Row.names), -merged_data$NumberofMurders, sum), y = merged_data$Murder)) +   # reorder attribute is used to sort the graph or chart in ascending or descending. -merged_data$Murder, sum parameters are used where the - indicates the graph is order in descending starting from 0 in x axis by murder rate.
...
bar_chart_murder_state_filled <- ggplot() + geom_col(data = merged_data, aes(x = reorder(factor(Row.names), -merged_data$NumberofMurders, sum), y = merged_data$Murder, fill=merged_data$PCNT_POPEST18PLUS)) +  # fill attribute is used to have the bar chart color filled by Percentage of population over 18. the color gradient varies by the magnitude of value of percentage.
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ggtitle("Total Murders") +
  labs(x="State",y="Murder rate") + 
  theme(plot.title = element_text(color="#666666", face="bold", size=15, hjust=0)) +
  theme(axis.title = element_text(color="#666666", face="bold", size=15))
